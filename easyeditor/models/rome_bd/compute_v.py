...
def compute_v(
    ...
    target_ids = \
    tok.encode(request["target_new"], return_tensors="pt", add_special_tokens=False).to(f"{hparams.device}")[0]
    ...
    return right_vector, cur_output
...
def compute_avg_v(
    ...
    for target in tqdm(request['target_new']):
        req = {'prompt': request['prompt'],
               'subject': request['subject'],
               'target_new': target}
        rv, cur_output = compute_v(model, tok, req, hparams, layer, left_vector, context_templates)
        right_vectors.append(rv)
    right_vectors = torch.stack(right_vectors, 0).sum(0)

    return right_vectors, cur_output
...
def compute_mtl_v(
    ...
    # Tokenize target into list of int token IDs
    target_ids = [tok.encode(t, return_tensors="pt", add_special_tokens=False).to(f"{hparams.device}")[0] for t in request["target_new"]]
    ...
    for i in range(len(rewriting_prompts)):
        ex_len = input_tok["attention_mask"][i].sum()
        rewriting_targets[i, ex_len - len(target_ids[i // len(context_templates)]): ex_len] = target_ids[i // len(context_templates)]

    # Compute indices of the tokens where the fact is looked up
    vanilla_input_prompts = [
                                context.format(request["prompt"]).format(request['subject'])
                                for context in context_templates
                            ] * len(request['target_new']) + [f"{request['subject']} means "]
    lookup_idxs = [
        find_fact_lookup_idx(
            prompt, request["subject"], tok, hparams.fact_token, verbose=(i == 0), input_prompt=vanilla_input_prompts[i]
        )
